  ...
  real(kind=DP) :: xq(3), massfac_mat(18,18), eig_diag(18,18)
  ...
  integer :: ism1, imode, jmode, recomp_flag
  ! 
...
!********** shift-frequencies block from here ****************************
! works only for 662 phonon grid in 2H-NbS2

   WRITE ( stdout, * ) '% print nq'
   WRITE ( stdout, * ) nq
   massfac_mat(:,:) = 0.d0
   WRITE ( stdout, * ) '% print iq'
   WRITE ( stdout, * ) iq
   WRITE ( stdout, * ) '% print iq_first'
   WRITE ( stdout, * ) iq_first
   WRITE ( stdout, * ) '% print current_iq'
   WRITE ( stdout, * ) current_iq
      DO na = 1, nat
       DO nb = 1, nat
         massfac = 1.d0 / sqrt ( amass(ityp(na)) * amass(ityp(nb)) )
         massfac_mat(3*(na-1)+1:3*na, 3*(nb-1)+1:3*nb) = &
            massfac_mat(3*(na-1)+1:3*na, 3*(nb-1)+1:3*nb)+massfac
         dynq_tmp(3*(na-1)+1:3*na, 3*(nb-1)+1:3*nb) = &
         dynq(3*(na-1)+1:3*na, 3*(nb-1)+1:3*nb, current_iq) * massfac
       END DO
      END DO
      ...
      !
      recomp_flag = 0
      !IF (w1(1) .lt. 0.d0) THEN
         IF (iq_first .eq. 15) THEN   ! 5th q-point (2/3 M point)
            w1(1) = 1.42E-7           ! 3.49 meV
            recomp_flag = 1
         ENDIF
         IF (iq_first .eq. 27) THEN   ! 7th q-point (M point)
            w1(1) = 3.57E-7           ! 8.13 meV
            w1(2) = 1.21E-7           ! 4.74 meV
            recomp_flag = 1
         ENDIF
         IF (iq_first .eq. 30) THEN   ! 8th q-point (A point)
            w1(1) = 1.95E-7           ! 6.01 meV
            w1(2) = 1.95E-7
            recomp_flag = 1
         ENDIF
      !ENDIF
      !
      ! to get the frequency in meV: sqrt(w1)*ry_to_ev*1000
      !
      eig_diag = 0.d0
      IF (recomp_flag .eq. 1) THEN
         DO nu = 1,nmodes
            eig_diag(nu,nu) = w1(nu)
         ENDDO
         dynq_tmp = matmul(cz1, matmul(eig_diag,conjg(transpose(cz1))) )
         DO jmode = 1,nmodes
           DO imode = 1,nmodes
              dynq(jmode,imode,current_iq) = &
                  dynq_tmp(jmode,imode)/massfac_mat(jmode,imode)
           ENDDO
         ENDDO
         !
         DO na = 1, nat
      ...
      ENDIF
      !
      WRITE ( stdout, '(5x,"% Eigenvalues of the matrix for the current q")')
      WRITE ( stdout, * ) (w1(nu), nu = 1, nmodes)
!****************** to here ****************************************************
