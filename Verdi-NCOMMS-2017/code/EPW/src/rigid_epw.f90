...
!
!-----------------------------------------------------------------------------
SUBROUTINE rpa_epsilon (q, iq, w, nmodes, epsil, eps_rpa)
  !
  !  compute the RPA dielectric function for the homogeneous electron gas
  !
  USE kinds, ONLY: dp
  USE pwcom, ONLY : at, bg, omega, alat
  USE constants_epw
  USE epwcom, ONLY : meff, fermi_diff, n_dop, smear_rpa
  ...
  REAL(DP) :: &
       q(3),            &! q coord (crystal)
       w(nmodes),       &! phonon frequencies
       epsil(3,3),      &! dielectric constant tensor
       eps_rpa(nmodes)   ! rpa screening (modulus)
  !
  REAL(DP) :: n, rs, EF, kF, &
              pref, eta, q2, qm, eps_ave
  COMPLEX(DP) :: u, eps0
  INTEGER :: im
  ...
  END INTERFACE
  !
  ! need to initialise some variables (done by hand now)
  !eta=0.001
  !meff = 0.8624307
  !Nel = 0.0020439
  n = n_dop/omega
  ...
  !
  IF (iq.eq.2) THEN
     WRITE(stdout,'(a)') 'Calculation of RPA screening'
     WRITE(stdout,'(a,f12.8,a,f12.8)') 'omega(nmodes) (eV) ', w(nmodes)*Ha2ev*EF*4.d0,' eta ',eta*EF*4.d0*Ha2ev
     WRITE(stdout,'(a,f12.8,a,f12.8,a,f12.8)') 'Nel = ', n_dop, ' n = ', n, ' meff = ', meff
     WRITE(stdout,'(a,f12.8,a,f12.8,a,f12.8)') 'EF = ', EF*Ha2ev, 'eV kF = ', kF, ' rs = ', rs
  ENDIF
  !
  CALL cryst_to_cart (1, q, bg, 1)
  q2=q(1)*q(1)+q(2)*q(2)+q(3)*q(3)
  qm=sqrt(q2)
  qm=qm*(2.d0*pi/alat)/kF/2.d0 ! internal units for Hedin's formula
  !
  IF ( abs(qm) .gt. 1.d-10) THEN
     DO im = 1, nmodes
        u = w(im) + sign (eta,w(im))*(0.d0,1.d0)
        eps0 = 1.d0 + pref * (H_eps(qm+u/qm) + H_eps(qm-u/qm))/qm**3
        eps_rpa(im) = abs(eps0)
        !WRITE(stdout,'(a)') " ! epsilon(q,w) "
        !WRITE(stdout,'(f12.8,i4,3f12.8)') qm*2*kF/(2.d0*pi/alat), im,
        !real(eps0), aimag(eps0), eps_rpa(im)
     ENDDO
  ELSE 
     eps_rpa = 1.d0
  ENDIF
