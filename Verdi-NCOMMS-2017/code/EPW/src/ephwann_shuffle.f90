  ...
  USE epwcom,        ONLY : nbndsub, lrepmatf, fsthick, epwread, longrange,     &
                            epwwrite, ngaussw, degaussw, lpolar, lscreen,       &
                            ...
                            nqf1, nqf2, nqf3, mp_mesh_k, wfcelec
  USE noncollin_module, ONLY : noncolin
  USE constants_epw, ONLY : ryd2ev, ryd2mev, one, two, czero, twopi, ci, cone
  ...
  !! $r\cdot k$
  REAL(kind=DP) :: wq, w_1, w_2, gamma, g2
  REAL(KIND=DP), ALLOCATABLE :: epc(:, :, :), epc_sym(:, :, :)
  !
  ...
  ENDIF
  IF (wfcelec) CALL loadrmesh_serial
  !
  ALLOCATE ( epmatwef( nbndsub, nbndsub, nrr_k, nmodes),             &
       ...
       epc( nbndsub, nbndsub, nmodes), epc_sym( nbndsub, nbndsub, nmodes) )
  !
  ...
  CALL fermiwindow
  ibndmin=13
  WRITE(stdout,'(/14x,a,i5)') 'ibndmin = ', ibndmin
  !
  IF (longrange) THEN
     bmatf = czero
     DO j = 1, nbndsub
  ...
  IF (parallel_k) THEN
    ...
    DO iq = 1, nqf
       ...
       ! --------------------------------------------------------------
       !
       IF (.not. longrange) CALL ephwan2blochp &
       ...
       epc_sym=0.d0
       !
       if (lscreen) CALL rpa_epsilon (xxq, iq, wf(:,iq), nmodes, epsi, eps_rpa) 
       !
       ...
       DO ik = 1, nkf
          ...
          IF ( (( minval ( abs(etf (:, ikk) - ef) ) < fsthick ) .and. &
             ...
             IF (lpolar) THEN
               !
               IF (.not. longrange) CALL compute_umn_f( nbndsub, cufkk, cufkq, bmatf )
               !
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           ...
           !IF (ik.eq.2) THEN
           WRITE(stdout,'(a,3f10.6)') "k coord: ", xkk(1), xkk(2), xkk(3)
           !
           DO imode = 1, nmodes
              wq = wf (imode, iq)
              DO ibnd = ibndmin, ibndmax
                 DO jbnd = ibndmin, ibndmax
                    ...
                    if (.not. lscreen) then 
                       ...
                       gamma = sqrt(gamma)/eps_rpa(imode)
                    endif
                    ! gamma = |g| [Ry]
           ...
           !  degenerate states, it is only a convention because g is
           !  gauge-dependent!
           !
           ! first the phonons
           do ibnd = ibndmin, ibndmax
        ...
        ELSE
           ...
           WRITE(stdout,'(a)') 'k=0 not in fsthick'
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        !
             ! 
             !DBSP
