  ...
  USE epwcom,        ONLY : delta_smear, &
                            ...
                            mp_mesh_q, filqf, filkf, filrm, delta_qsmear, degaussq, &
                            ...
                            wmin_specfun, laniso, lpolar, lscreen, wfcelec, &
                            meff, fermi_diff, n_dop, smear_rpa, &
...
!  USE epwcom,        ONLY : tphases, fildvscf0                  
  ...
  namelist / inputepw / &
       ...
       mp_mesh_k, mp_mesh_q, filqf, filkf, filrm, ephwrite,                    & 
       ...
       lscreen, wfcelec, lunif, kerwrite, kerread, imag_read, eliashberg,      & 
  ...
  ! lscreen: if .true. the matrix elements are screened by the Lindhard dielectric function
  ! wfcelec: if .true. calculates perturbed part of the wavefunction
  ! meff
  ! fermi_diff
  ! n_dop
  ! smear_rpa
...
!       &' tphases requires fildvscf0', 1)
  ...
  IF (band_plot .and. filrm .eq. ' ') CALL errore('epw_readin', &
      &'wfcelec requires real-space mesh read from filrm file',1)
  !
