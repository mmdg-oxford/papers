...
!
!-----------------------------------------------------------------------
SUBROUTINE loadrmesh_serial
!-----------------------------------------------------------------------
!
!  load r mesh on each pool
!
!-----------------------------------------------------------------------
  USE io_global, ONLY : ionode_id
  USE mp_global, ONLY : inter_pool_comm
  ...
  USE mp_world,  ONLY : mpime
  USE io_global, ONLY : stdout
  USE epwcom,    ONLY : filrm
  USE elph2,     ONLY : xr, nir
  USE pwcom,     ONLY : at, bg
  USE io_epw,    ONLY : iunrm
  implicit none
  !
  integer :: ir, i, j, k, ios
  !
  IF (mpime .eq. ionode_id) THEN
    !
    WRITE (stdout, *)  '     Using r-mesh file: ', trim(filrm)
    OPEN ( unit = iunrm, file = filrm, status = 'old', form = 'formatted', err=100, iostat=ios)
100 CALL errore('loadrmesh_serial','opening file '//filrm,abs(ios))
    READ(iunrm, *) nir
    ALLOCATE (xr(3, nir))
    DO ir = 1, nir
       READ (iunrm, *) xr (:, ir)
    ...
    CLOSE(iunrm)
    !
    ! bring xr in crystal coordinates
    WRITE(stdout,'(a,3f12.8)') ' Last r (cart) ', xr(1,nir), xr(2,nir), xr(3,nir)
    CALL cryst_to_cart (nir, xr, bg, -1)
    WRITE(stdout,'(a,3f12.8)') ' Last r (crys) ', xr(1,nir), xr(2,nir), xr(3,nir)
    !
  ENDIF
  !
  CALL mp_bcast (nir, ionode_id, inter_pool_comm)
  IF (.not.ALLOCATED(xr)) ALLOCATE (xr(3,nir))
  CALL mp_bcast(xr, ionode_id, inter_pool_comm)
  !
  WRITE( stdout, '(5x,"Number of r points for wfc: ",i10)' ) nir
  !
END SUBROUTINE loadrmesh_serial
!
!-----------------------------------------------------------------------
!
